...
export default function Pagination({ numPage, numActive, path }) {
    ...
    for (let i = 1; i <= numPage; i++) {
        if (i == numActive) {
            li.push(
                <Link className='text-[#b2becd] bg-[#0e1417] md:py-[16px] md:px-[32px] py-[13px] px-[26px]  text-[15px]' href={{ pathname: path, query: { page: i } }}>
                    {i}
                </Link>
            )
            continue;
        ...
        li.push(
            <Link className='text-[#b2becd] bg-[#0d1113] md:py-[16px] md:px-[32px] py-[13px] px-[26px]  text-[15px]' href={{ pathname: path, query: { page: i } }}>
                {i}
            </Link>
        )
    }
